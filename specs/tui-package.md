...
Move the canonical Shob terminal application from
`packages/shob/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/shob`,
`packages/cli`, or `@shob/core`.

The SDK is the TUI's Shob boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/shob`.
- Do not replace private `packages/shob` imports with `@shob/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@shob/tui`.
...
- Shob domain data displayed by the TUI
...
```text
packages/shob/src/cli/cmd/tui
```

Its private dependency on `packages/shob` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/shob/src/*`.
...
Status: Completed. The private `@shob/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@shob/tui`.
...
- The package has no dependency on `shob`, `@shob/cli`, or
  `@shob/core`.
...
primitives now live in `@shob/tui`. The legacy host consumes explicit
...
and build inputs through `@shob/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Shob globals or process state;
...
Replace process-global Shob state with resolved TUI inputs.
...
`@shob/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/shob` initially.
...
canonical in `@shob/tui`. Configured references resolve through the new
...
adapters rather than Shob domain implementations.

Make the SDK the only Shob domain boundary used by the TUI.
...
- Domain-facing TUI code imports Shob data and operations only from
  `@shob/sdk`.
...
`@shob/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/shob`.
...
- The package has no imports from `packages/shob`, `packages/cli`, or
  `@shob/core`.
...
public `@shob/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/shob`.
- Keep the legacy embedded worker and server startup in `packages/shob`.
...
- Remove obsolete `@tui/*` path mappings from `packages/shob`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@shob/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@shob/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Shob domain state.
...
cd packages/shob && bun typecheck
...
rg '@shob/core|packages/shob|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/shob/src packages/shob/test
```
...
- Build the current-platform `packages/shob` binary.
