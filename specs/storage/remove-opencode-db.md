# Remove `packages/shob/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/shob/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the shob legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/shob/src/storage/db.ts` are concentrated in 22 source files:

- `packages/shob/src/account/repo.ts`
- `packages/shob/src/cli/cmd/db.ts`
- `packages/shob/src/cli/cmd/import.ts`
- `packages/shob/src/cli/cmd/stats.ts`
- `packages/shob/src/control-plane/workspace.ts`
- `packages/shob/src/index.ts`
- `packages/shob/src/node.ts`
- `packages/shob/src/permission/index.ts`
- `packages/shob/src/project/project.ts`
- `packages/shob/src/server/projectors.ts`
- `packages/shob/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/shob/src/server/shared/fence.ts`
- `packages/shob/src/session/message-v2.ts`
- `packages/shob/src/session/projectors.ts`
- `packages/shob/src/session/prompt.ts`
- `packages/shob/src/session/session.ts`
- `packages/shob/src/session/todo.ts`
- `packages/shob/src/share/share-next.ts`
- `packages/shob/src/storage/db.ts`
- `packages/shob/src/sync/index.ts`
- `packages/shob/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy shob database wrapper; `packages/shob/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the shob wrapper.
...
Status: Completed. `SyncEvent` and the shob projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy shob database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy shob database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy shob database wrapper.
...
All migration groups are complete or superseded. `packages/shob/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No shob data-migration group remains.

The previous shob `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate shob data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/shob`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/shob/src`
- `bun typecheck` from `packages/shob`
- Relevant package tests from `packages/shob`, not the repo root
