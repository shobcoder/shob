...
Both `action` and `resource` use shob's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.shob` files is intentionally deferred until `.shob` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether shob permits an operation through its managed execution paths.
