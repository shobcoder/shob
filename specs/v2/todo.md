...
The shob server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@shob/llm` tool loop after replacing its
...
- shob instance has stuff like `opencode.session.prompt()` or
...
- decide whether to preserve deprecated `@shob/llm` orchestration exports
