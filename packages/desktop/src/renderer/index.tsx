...
import "./styles.css"
import { Splash } from "@shob/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.SHOB_CHANNEL === "prod" &&
...
// Chromium emits this when layout is still settling; it's benign and was flooding the
// session timeline (virtualizer + streaming). Root thrash is rAF-batched in the app.
if (typeof window !== "undefined") {
  const isResizeObserverLoop = (value: unknown) =>
    typeof value === "string" && value.includes("ResizeObserver loop")
  const originalError = console.error.bind(console)
  console.error = (...args: unknown[]) => {
    if (args.some(isResizeObserverLoop)) return
    originalError(...args)
  }
...
const deepLinkEvent = "shob:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__SHOB__ ??= {}
  const pending = window.__SHOB__.deepLinks ?? []
  window.__SHOB__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `shob.desktop.window.${windowID}.last-active-url`
}
...
const createPlatform = (windowState: DesktopWindowState): Platform => {
  ...
  return {
    ...
    listSkillStore: () => window.api.listSkillStore(),
    installSkill: (skillId) => window.api.installSkill(skillId),
    uninstallSkill: (skillId) => window.api.uninstallSkill(skillId),
    ...
    browserAction: (request) => window.api.browserAction(request),
    onBrowserState: (cb) => window.api.onBrowserState(cb),
    onBrowserOpen: (cb) => window.api.onBrowserOpen(cb),
    onBrowserClose: (cb) => window.api.onBrowserClose(cb),
  }
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("shob.global.dat").getItem("language")
