...
const api: ElectronAPI = {
  ...
  wslServers: {
    ...
    installOpencode: (name) => ipcRenderer.invoke("wsl-servers-install-shob", name),
  ...
  browserAction: (request) => ipcRenderer.invoke("browser-action", request),
  onBrowserState: (cb) => {
  ...
  onBrowserClose: (cb) => {
    ...
    return () => ipcRenderer.removeListener("browser-close", handler)
  },

  listSkillStore: () => ipcRenderer.invoke("list-skill-store"),
