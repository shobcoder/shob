...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Shob update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Shob", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.shob/bin/shob",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, shobChecks: {} })
})
...
test("ignores stale background Shob checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().shobChecks).toEqual({})
})

test("ignores stale startup Shob checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Shob", async () => {
  persistedServers = []
  ...
  expect(shob).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().shobChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Shob in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const shob: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(shob).toEqual(["Debian"])
  expect(Object.keys(controller.getState().shobChecks)).toEqual(["Debian"])
})
