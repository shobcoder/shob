...
export async function spawnWslSidecar(
  ...
  const shob = await resolveWslOpencode(distro)
  if (!shob) throw new Error(`Shob is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export SHOB_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SHOB_CLIENT=desktop",
    `export SHOB_SERVER_USERNAME=${shellEscape(username)}`,
    `export SHOB_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(shob)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
