...
const main = Effect.gen(function* () {
  ...
  const onboardingTestRoot = ((): string | undefined => {
    ...
    const root = join(tmpdir(), `shob-onboarding-${randomUUID()}`)
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("shob://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("shob")
  registerRendererProtocol()
  setDockIcon()
  const updater = setupAutoUpdater(stopSidecars)
  browserControl = createBrowserControl({
    getWindow: getLastFocusedWindow,
    emit: (win, channel, payload) => {
      const target = win && !win.isDestroyed() ? win : getLastFocusedWindow()
      if (!target || target.isDestroyed()) return
      target.webContents.send(channel.replace("browser:", "browser-"), payload)
    },
  })
  const browserEndpoint = yield* Effect.promise(() => browserControl!.start())
  process.env.SHOB_BROWSER_CONTROL_URL = browserEndpoint.url
  process.env.SHOB_BROWSER_CONTROL_TOKEN = browserEndpoint.token
  process.env.OPENCODE_BROWSER_CONTROL_URL = browserEndpoint.url
  process.env.OPENCODE_BROWSER_CONTROL_TOKEN = browserEndpoint.token
  registerIpcHandlers({
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.SHOB_PORT
    if (fromEnv) {
