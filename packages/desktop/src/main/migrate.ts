...
// `shob.settings.dat` is special: it maps to the `shob.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"shob.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // shob.settings.dat → the electron settings store ("shob.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "shob.global.dat").
  const storeName = filename === "shob.settings.dat" ? "shob.settings" : filename
