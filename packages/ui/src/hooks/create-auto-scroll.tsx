...
export function createAutoScroll(options: AutoScrollOptions) {
  ...
  createResizeObserver(
    () => store.contentRef,
    () => {
      // Coalesce RO notifications to one rAF — avoids ResizeObserver loop spam while streaming.
      ...
      contentResizeFrame = requestAnimationFrame(() => {
        ...
        if (!active()) return
        if (store.userScrolled) return
        // Keep the bottom locked without re-entering layout in the RO callback.
        scrollToBottom(false)
      })
    },
  )
