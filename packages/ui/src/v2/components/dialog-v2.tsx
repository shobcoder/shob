...
export function DialogTitleGroup(props: DialogTitleGroupProps) {
  ...
  return (
    <div data-slot="dialog-title-group">
      <Show when={icon()}>{(i) => <div data-slot="dialog-title-icon">{i()}</div>}</Show>
      <div data-slot="dialog-title-text">
...
export function DialogHeader(props: DialogHeaderProps) {
  const [local] = splitProps(props, ["closeLabel", "hideClose", "icon", "children"])
  const hideClose = () => local.hideClose === true

  return (
    <div data-slot="dialog-header" data-hide-close={hideClose() ? "" : undefined}>
      <Show when={local.icon}>{(i) => <div data-slot="dialog-header-icon">{i()}</div>}</Show>
      <div data-slot="dialog-header-content">{local.children}</div>
      {!hideClose() && (
