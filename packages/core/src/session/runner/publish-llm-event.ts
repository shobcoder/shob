import { ToolOutput, ToolContent, type LLMEvent, type ProviderMetadata, type ToolResultValue, type Usage } from "@shob/llm"
...
import { ToolEventJournal } from "../tool-journal"
...
export const createLLMEventPublisher = (
  ...
  const toolEvent = <D extends Definition>(definition: D, data: Data<D>, options?: PublishOptions): Effect.Effect<void> =>
    journal ? journal.append({ definition, data, options }) : events.publish(definition, data, options).pipe(Effect.asVoid)
  ...
  const toolInput = fragments("tool input", (callID, value) =>
    Effect.gen(function* () {
      ...
      yield* toolEvent(SessionEvent.Tool.Input.Ended, {
        sessionID: input.sessionID,
        timestamp: yield* timestamp,
  ...
  const startToolInput = Effect.fnUntraced(function* (event: { readonly id: string; readonly name: string }) {
    ...
    yield* toolEvent(SessionEvent.Tool.Input.Started, {
      sessionID: input.sessionID,
      timestamp: yield* timestamp,
      assistantMessageID,
      callID: event.id,
  ...
  const failUnsettledTools = Effect.fn("SessionRunner.failUnsettledTools")(function* (
    ...
    for (const [callID, tool] of tools) {
      ...
      tool.settled = true
      yield* toolEvent(SessionEvent.Tool.Failed, {
        sessionID: input.sessionID,
        timestamp: yield* timestamp,
        assistantMessageID: tool.assistantMessageID,
        callID,
  ...
  const publish = Effect.fn("SessionRunner.publishLLMEvent")(function* (
    ...
    switch (event.type) {
      ...
      case "tool-input-delta": {
        ...
        yield* toolEvent(SessionEvent.Tool.Input.Delta, {
          sessionID: input.sessionID,
          timestamp: yield* timestamp,
          assistantMessageID: tool.assistantMessageID,
          callID: event.id,
      ...
      case "tool-call": {
        ...
        yield* toolEvent(SessionEvent.Tool.Called, {
          sessionID: input.sessionID,
          timestamp: yield* timestamp,
          assistantMessageID: tool.assistantMessageID,
          callID: event.id,
      ...
      case "tool-result": {
        ...
        yield* toolEvent(SessionEvent.Tool.Success, {
          sessionID: input.sessionID,
          timestamp: yield* timestamp,
          assistantMessageID: tool.assistantMessageID,
          callID: event.id,
  ...
  const progress = Effect.fnUntraced(function* (
    ...
    const assistantMessageID = yield* assistantMessageIDForTool(callID)
    yield* toolEvent(SessionEvent.Tool.Progress, {
      sessionID: input.sessionID,
      timestamp: yield* timestamp,
      assistantMessageID,
      callID,
