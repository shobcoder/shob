...
import { ToolFailure } from "@shob/llm"
...
import { Tool, type Context } from "./tool"
...
const streamShell = (args: {
  ...
  Effect.gen(function* () {
    const callID = args.context.toolCallID
    const emit = (structured: Record<string, unknown>, text: string) =>
      args.context.progress?.({ callID, structured, content: text ? [{ type: "text", text }] : [] }) ?? Effect.void
    yield* emit({ started: true }, "")
    const child = ChildProcess.make(args.command, [], {
      cwd: args.cwd,
      shell: args.shell,
      stdin: "ignore",
    ...
    let total = 0
    let truncated = false
    const timedOut = { _tag: "timeout" as const }
    const outcome = yield* args.appProcess
      .runStream(child, { includeStderr: true, okExitCodes: [0] })
      .pipe(
        Stream.tap((line) =>
          Effect.gen(function* () {
            if (truncated) return
            const add = line + "\n"
            if (total + add.length > MAX_CAPTURE_BYTES) {
              truncated = true
              return
            }
            total += add.length
            full += add
            buffer += add
            if (buffer.length >= args.chunkBytes) {
              yield* emit({ output: buffer }, buffer)
              buffer = ""
            }
          }),
        ),
        ...
        Effect.match({
          onFailure: (error) => ({ exitCode: error.exitCode ?? 1, timedOut: false }),
          onSuccess: () => ({ exitCode: 0, timedOut: false }),
        }),
        Effect.timeoutOrElse({
          duration: Duration.millis(args.timeout),
          orElse: () => Effect.succeed({ exitCode: undefined, timedOut: true }),
        }),
      )
    if (buffer.length > 0) yield* emit({ output: buffer }, buffer)
    if (outcome.timedOut)
      return {
        output: `Command exceeded timeout of ${args.timeout} ms. Retry with a larger timeout if the command is expected to take longer.`,
        truncated: false,
        timeout: true,
        ...
        [name]: Tool.make({
          ...
          execute: (input, context) =>
            Effect.gen(function* () {
              ...
              // Streaming path: emit live output and exit metadata as Tool.Progress events.
              if (context.progress) {
                return yield* streamShell({
                  ...
                  context,
                  warnings,
                  chunkBytes: ConfigToolExecution.resolve(Config.latest(entries, "tool_execution") ?? undefined)
                    .shellChunkBytes,
                  appProcess,
                })
              }
