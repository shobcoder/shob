...
export class ConfigService extends Context.Service<ConfigService, Config>()("@shob/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SHOB_WEBSEARCH_PROVIDER === "exa" || process.env.SHOB_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SHOB_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SHOB_EXPERIMENTAL") || truthy("SHOB_ENABLE_EXA") || truthy("SHOB_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SHOB_ENABLE_PARALLEL") || truthy("SHOB_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `shob/${InstallationVersion}`,
