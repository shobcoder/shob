...
import { ToolContent, ToolDefinition, ToolFailure, ToolOutput, type ToolCall } from "@shob/llm"
...
export interface Context {
  ...
  /** Optional live progress emitter. Present only when the runner settles a tool call; tools that stream output call it. */
  readonly progress?: (event: ToolProgress) => Effect.Effect<void>
}

/** Live progress payload a tool may emit during execution. The final result still comes from the tool's return value. */
