...
export class Info extends Schema.Class<Info>("Config.Info")({
  ...
  tool_execution: ConfigToolExecution.Info.pipe(Schema.optional).annotate({
    description: "High-speed tool execution, buffered persistence, and streaming shell tuning.",
  }),
...
export class Service extends Context.Service<Service, Interface>()("@shob/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["shob.json", "shob.jsonc"]
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".shob")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".shob").toReversed()
    ...
    // global config, project files, then `.shob` files.
