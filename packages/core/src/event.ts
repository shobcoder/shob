...
import type { Data, Definition, Payload } from "@shob/schema/event"
...
import { Durable } from "@shob/schema/durable-event-manifest"

export const ID = Event.ID
export type ID = import("@shob/schema/event").ID
export type { Data, Definition, Payload } from "@shob/schema/event"
...
export interface Interface {
  ...
  /** Persist an ordered batch of events in a single transaction and notify local subscribers afterwards. */
  readonly publishBatch: (
    events: ReadonlyArray<{ readonly definition: Definition; readonly data: Data<Definition>; readonly options?: PublishOptions }>,
...
export class Service extends Context.Service<Service, Interface>()("@shob/Event") {}
        ...
        return Effect.gen(function* () {
          ...
          const payloads = events.map((event) => {
            ...
            return {
              ...
              payload: {
                id: event.options?.id ?? ID.create(),
                ...(event.options?.metadata ? { metadata: event.options.metadata } : {}),
                type: event.definition.type,
                ...(location ? { location } : {}),
                data: event.data,
              } as Payload,
          ...
          const durableEntries = payloads.flatMap((entry) => {
            const durableInfo = entry.definition.durable
            return durableInfo == null ? [] : [{ entry, durableInfo }]
          })
          if (durableEntries.length === 0) {
            for (const entry of payloads) yield* notify(entry.payload, false)
            return payloads.map((entry) => entry.payload)
          }
          const byAggregate = new Map<string, typeof durableEntries>()
          for (const { entry, durableInfo } of durableEntries) {
            const info = durableInfo as { readonly version: number; readonly aggregate: string }
            const type = entry.definition.type as string
            const rawID = (entry.payload.data as Record<string, unknown>)[info.aggregate]
            if (typeof rawID !== "string")
              yield* Effect.die(
                new InvalidDurableEventError({
                  type,
                  message: `Expected string aggregate field ${info.aggregate}`,
                }),
              )
            const aggregateID = rawID as string
            const group = byAggregate.get(aggregateID) ?? []
            group.push({ entry, durableInfo })
            byAggregate.set(aggregateID, group)
          }
          ...
          for (const [aggregateID, group] of byAggregate) {
            const results = yield* db
              .transaction(() =>
                Effect.gen(function* () {
                  const row = yield* db
                    ...
                    .get()
                    .pipe(Effect.orDie)
                  let latest = row?.seq ?? -1
                  const out: Payload[] = []
                  for (const { entry, durableInfo } of group) {
                    latest += 1
                    const seq = latest
                    const encoded = Schema.encodeUnknownSync(entry.definition.data)(entry.payload.data) as Record<
                      string,
                      unknown
                    >
                    const committedEvent = {
                      ...entry.payload,
                      durable: { aggregateID, seq, version: durableInfo.version },
                    } as Payload
                    const list = projectors.get(entry.definition.type as string) ?? []
                    for (const projector of list) yield* projector(committedEvent)
                    if (entry.options?.commit) yield* entry.options.commit(seq)
                    yield* db
                      .insert(EventSequenceTable)
                      ...
                      .values([
                        {
                          ...
                          aggregate_id: aggregateID,
                          seq,
                          type: versionedType(entry.definition.type as string, durableInfo.version),
                          data: encoded,
                        },
                      ])
                      .run()
              ...
              .pipe(Effect.orDie)
            for (const wake of pubsub.durable.get(aggregateID) ?? []) yield* PubSub.publish(wake, undefined)
            committed.push(...results)
          }
          for (const entry of payloads) yield* notify(entry.payload, entry.definition?.durable ? true : false)
          return payloads.map((entry) => entry.payload)
        })
      }
