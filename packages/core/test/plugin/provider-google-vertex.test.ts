...
import { GoogleVertexPlugin } from "@shob/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.shob, (provider) => {
          provider.api = {
            ...
            url: "https://shob.olova.dev/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.shob))
