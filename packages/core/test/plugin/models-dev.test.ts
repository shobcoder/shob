...
import { ModelsDevPlugin } from "@shob/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.SHOB_MODELS_PATH,
          disabled: Flag.SHOB_DISABLE_MODELS_FETCH,
        }
        Flag.SHOB_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.SHOB_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
          ...
          expect(yield* integrations.list()).toEqual([
            ...
            new Integration.Info({
              id: Integration.ID.make("commandcode"),
              name: "Command Code",
              methods: [
                { type: "key" },
                {
                  type: "env",
                  names: ["COMMANDCODE_API_KEY"],
                },
              ],
              connections: [],
            }),
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.SHOB_MODELS_PATH = previous.path
          Flag.SHOB_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
