...
const shobClient = process.env.SHOB_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (shobClient === undefined) delete process.env.SHOB_CLIENT
  else process.env.SHOB_CLIENT = shobClient
})
...
test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "shob-log-test-"))
  await using _ = {
