...
> current private `@shob/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@shob/ai`
...
- Secondary audience: Shob and other durable agent runtimes
...
import { LLM } from "@shob/ai"
import { OpenAI } from "@shob/ai/providers/openai"
...
Shob and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@shob/ai/promise"
import { OpenAI } from "@shob/ai/providers/openai"
...
```ts
import { LLMSchema } from "@shob/ai/schema"
...
```ts
import { Provider, Protocol } from "@shob/ai/provider"
...
@shob/ai/providers/openai
@shob/ai/providers/anthropic
@shob/ai/providers/google
...
```
...
Shob should migrate to `generateTurn` / `streamTurn`, preserving its durable
