...
import { Tag } from "@shob/ui/v2/badge-v2"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.shobChecks[item.config.distro]
          const shobAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-shob" && wsl.data.job.distro === item.config.distro
          return (
