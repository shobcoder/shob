...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Shob needs attention", () => {
    ...
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/shob",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Shob is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        shobChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.shob/bin/shob",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.primaryButton.action).toBe("install-shob")
  })
