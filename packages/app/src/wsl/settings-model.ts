...
export function wslOpencodeAction(check?: WslOpencodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Shob"
  if (check.matchesDesktop === false) return "Update Shob"
}
...
export function addServerViewModel(input: {
  ...
  const shobCheck = selectedDistro ? (state?.shobChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.shobChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.shobMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  shobCheck: WslOpencodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-shob" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerOpencodeReady(input.shobCheck)) {
    const update = !!input.shobCheck?.resolvedPath && input.shobCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.shobChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.shobChecks[item.name]) return [`shob:${item.name}`]
    return []
  })
