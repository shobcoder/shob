...
type PlatformBase = {
  ...
  /** Control native embedded browser containers (desktop only) */
  browserAction?(request: BrowserActionRequest): Promise<BrowserActionResult>

  /** Subscribe to native embedded browser state updates (desktop only) */
  onBrowserState?(cb: (state: BrowserState) => void): () => void
  onBrowserOpen?(cb: (state: BrowserState) => void): () => void
  onBrowserClose?(cb: (payload: { browserId: string }) => void): () => void

  /** Built-in skill store (desktop only): catalog + install into ~/.shob/skills */
  listSkillStore?(): Promise<SkillStoreItem[]>
  installSkill?(skillId: string): Promise<SkillStoreItem>
