import type { Project, UserMessage } from "@shob/sdk/v2"
...
import { isScrollKeyTarget, scrollKey, scrollKeyOwner } from "@shob/ui/scroll-view"
import { Tabs } from "@shob/ui/tabs"
import { ButtonV2 } from "@shob/ui/v2/button-v2"
import { createAutoScroll } from "@shob/ui/hooks"
import { previewSelectedLines } from "@shob/session-ui/pierre/selection-bridge"
import { Button } from "@shob/ui/button"
import { TooltipKeybind } from "@shob/ui/tooltip"
import { showToast } from "@/utils/toast"
import { base64Encode, checksum } from "@shob/core/util/encode"
...
export default function Page() {
  ...
  const useTitlebarPanelControls = createMediaQuery("(max-width: 1440px)")
  const [titlebarRightEl, setTitlebarRightEl] = createSignal<HTMLElement | null>(null)
  const size = createSizing()
  const browserSize = createSizing()
  ...
  const sessionPanelWidth = createMemo(() => {
    if (browserStackOpen()) {
      const browserWidth = browserStackWidth() + 16
      if (desktopSidePanelOpen() && desktopReviewOpen()) return `calc(${layout.session.width()}px - ${browserWidth}px)`
      if (desktopSidePanelOpen()) return `calc(100% - ${layout.fileTree.width()}px - ${browserWidth}px)`
      return `calc(100% - ${browserWidth}px)`
    }
  ...
  const toggleBrowserPanel = () => {
    setBrowserPanelVisible((visible) => {
      ...
      if (desktopReviewOpen()) return false
      return true
    })
  }
      ...
      <div class="flex shrink-0 items-center gap-0">
        ...
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" class="shrink-0" aria-hidden="true">
            ...
            />
            <path
              d="M21.17 8.75C21.12 8.75 21.06 8.74 21.01 8.73C18.07 8.09 15.1 8.09 12.16 8.73C11.75 8.82 11.36 8.56 11.27 8.16C11.18 7.75 11.44 7.36 11.84 7.27C14.99 6.58 18.18 6.58 21.33 7.27C21.73 7.36 21.99 7.76 21.9 8.16C21.83 8.51 21.52 8.75 21.17 8.75Z"
              fill="currentColor"
            />
            <path
              d="M8.54001 14.75C8.33001 14.75 8.12001 14.66 7.97001 14.49C5.89001 12.1 4.31001 9.37001 3.26001 6.37001L3.95001 6.06001L4.66001 5.82001L4.68001 5.88001C5.66001 8.69001 7.15001 11.26 9.11001 13.51C9.38001 13.82 9.35001 14.3 9.04001 14.57C8.89001 14.69 8.71001 14.75 8.54001 14.75Z"
              fill="currentColor"
            />
            <path
              d="M10.8801 22.69C10.7001 22.69 10.5201 22.63 10.3801 22.5C10.0701 22.22 10.0501 21.75 10.3301 21.44C12.3301 19.24 13.8101 16.69 14.7301 13.86C14.8601 13.47 15.3001 13.21 15.6901 13.34C16.0801 13.47 16.3101 13.85 16.1901 14.24C15.1801 17.36 13.5901 20.1 11.4501 22.46C11.2901 22.61 11.0801 22.69 10.8801 22.69Z"
              fill="currentColor"
            />
          </svg>
        </Button>
  ...
  let contentRoFrame: number | undefined
  createResizeObserver(
    () => content,
    () => {
      if (contentRoFrame !== undefined) return
      contentRoFrame = requestAnimationFrame(() => {
        contentRoFrame = undefined
        const el = scroller
        if (el) scheduleScrollState(el)
        fill()
      })
  ...
  let promptRoFrame: number | undefined
  createResizeObserver(
    () => promptDock,
    ({ height }) => {
      if (promptRoFrame !== undefined) return
      promptRoFrame = requestAnimationFrame(() => {
        promptRoFrame = undefined
  ...
  const sessionPanelContent = () => (
    <>
      ...
      <Show when={(params.id || false) && !mobileChanges()}>{(_) => composerRegion()}</Show>
    ...
    <SessionRouteFrame>
      ...
      >
        ...
        <div
          classList={{
            ...
            "duration-[240ms] ease-[cubic-bezier(0.22,1,0.36,1)] will-change-[width] motion-reduce:transition-none":
              !size.active() && !browserSize.active() && !ui.reviewSnap,
          }}
          style={{
        ...
        >
          ...
          <Show when={params.id && !useTitlebarPanelControls()}>
            <div class="pointer-events-none absolute right-3 top-2 z-40 flex items-center justify-end">
              <div class="pointer-events-auto flex items-center gap-0 rounded-[14px] bg-[color-mix(in_srgb,var(--background-stronger,var(--background))_78%,transparent)] p-0.5 backdrop-blur">
                {panelControls()}
              </div>
            </div>
          </Show>
          <Show when={params.id && useTitlebarPanelControls() ? titlebarRightEl() : undefined}>
            {(el) => (
              <Portal mount={el()}>
                <div class="flex h-full min-w-0 items-center pr-1">{panelControls()}</div>
              </Portal>
            )}
          </Show>
        ...
        <Show when={browserStackOpen()}>
          <div
            class="group relative z-40 hidden h-full w-0 shrink-0 cursor-col-resize items-center justify-center md:flex"
            classList={{
              "-ml-2": true,
            }}
            onPointerDown={browserSize.start}
            aria-hidden="true"
          >
            <ResizeHandle
              class="absolute top-0 h-full cursor-col-resize"
              direction="horizontal"
              edge="start"
              size={browserStackWidth()}
              min={300}
              max={typeof window === "undefined" ? 720 : Math.max(320, window.innerWidth * 0.55)}
              onResize={(width) => {
                browserSize.touch()
                setBrowserStackWidth(width)
              }}
              style={{ left: "-9px", width: "18px", transform: "none" }}
            />
            <div
              class="pointer-events-none h-14 w-1 rounded-full bg-border-strong-base opacity-0 transition-opacity group-hover:opacity-70"
              classList={{
                "opacity-90": browserSize.active(),
              }}
            />
          </div>
        </Show>
        ...
        <SessionBrowserStack
          ...
          width={browserStackWidth()}
          resizing={browserSize.active()}
          onRequestVisible={() => {
            if (desktopReviewOpen()) return
            setBrowserPanelVisible(true)
          }}
          onOpenChange={setBrowserStackOpen}
        />
