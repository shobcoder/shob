...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("shob://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("shob://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("shob://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("shob://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("shob://open-project")).toBeUndefined()
    expect(parseDeepLink("shob://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "shob://open-project?directory=/a",
      "shob://other?directory=/b",
      "shob://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("shob://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("shob://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("shob://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("shob://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "shob://new-session?directory=/a",
      "shob://open-project?directory=/b",
      "shob://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SHOB__: {
        deepLinks: ["shob://open-project?directory=/a"],
      },
    } as unknown as Window & { __SHOB__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["shob://open-project?directory=/a"])
