...
import { AssistantMessage, Part, SessionStatus, SnapshotFileDiff, UserMessage } from "@shob/sdk/v2"
import { groupParts, renderable, type PartGroup } from "@shob/session-ui/message-part"
import { activitySnapshotForVisibleParts } from "@shob/session-ui/session-activity"
...
export namespace Timeline {
  export function constructMessageRows(
    ...
    // Bottom-of-turn status while the agent is busy: Reading / Editing / Running / …
    // Prefer in-flight tools so completed work doesn't stick as the live label.
    if (isActive && status === "busy" && !error) {
      const allParts = assistantMessages.flatMap((message) => getMessageParts(message.id))
      // Use full part stream (not only renderable) so pending tools still count.
      const activity = activitySnapshotForVisibleParts(allParts, { now: Date.now() })
      const heading = allParts
