...
- Author package entrypoint is `@shob/plugin/tui`.
...
```json
{
  "$schema": "https://shob.olova.dev/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/shob-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@shob/plugin/tui`.
- `@shob/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@shob/plugin/tui"
...
- The value is a semver range checked against the running Shob version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.shob` is absent, no check is performed (backward compatible).
...
- `shob plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `shob plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.shob` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.shob`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.shob`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.shob` dir or `SHOB_CONFIG_DIR`, Shob installs `@shob/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@shob/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Shob's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Shob registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Shob pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@shob/plugin/tui`.
...
- Local plugins persist installed themes under the local `.shob/themes` area near the plugin config source.
...
- `--pure` / `SHOB_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.shob/plugins/tui-smoke.tsx`
- Local vim plugin: `.shob/plugins/tui-vim.tsx`
- Local smoke config: `.shob/tui.json`
- Local smoke theme: `.shob/plugins/smoke-theme.json`
