...
Trim `packages/shob/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/shob/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/shob`.
- `[x]` `bun typecheck` from `packages/shob`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/shob/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/shob`.
- `[x]` `bun dev generate > /tmp/shob-openapi.json` from `packages/shob`.
...
- Edit `packages/shob/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/shob-openapi.json` from `packages/shob`.
...
- Check source schemas in files like `packages/shob/src/session/schema.ts`, `packages/shob/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/shob/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/shob/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
