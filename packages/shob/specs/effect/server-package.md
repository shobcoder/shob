...
Practical reference for a future `packages/server` split after the shob
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/shob`.
...
Until enough shared service code lives outside `packages/shob`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/shob`

It should not import `packages/shob` services while `packages/shob`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on shob-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/shob` implementation details.
