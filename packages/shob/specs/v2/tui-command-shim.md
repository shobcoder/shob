...
Delete `packages/shob/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/shob/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/shob/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/shob`
- TUI plugin loader tests in `packages/shob` if runtime plugin API wiring changed
