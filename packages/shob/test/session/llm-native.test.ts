...
import { LLMEvent, ToolFailure } from "@shob/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@shob/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, shob, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored shob auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("opencode") },
        provider: {
