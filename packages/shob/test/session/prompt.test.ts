...
import { SessionMessageTable } from "@shob/core/session/sql"
...
import { Shell } from "@shob/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@shob/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://shob.olova.dev/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/shob/src/session/processor.ts")
