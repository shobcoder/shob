...
it.instance("plan agent denies edits except .shob/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".shob/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".shob", "skill", "perm-skill")
...
`,
      ...
      process.env.SHOB_TEST_HOME = test.directory
