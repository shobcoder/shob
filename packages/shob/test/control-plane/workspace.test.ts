...
import { EventSequenceTable } from "@shob/core/event/sql"
...
import { WorkspaceV2 } from "@shob/core/workspace"
import { WorkspaceTable } from "@shob/core/control-plane/workspace.sql"
...
const originalEnv = {
  SHOB_AUTH_CONTENT: process.env.SHOB_AUTH_CONTENT,
  SHOB_EXPERIMENTAL_WORKSPACES: process.env.SHOB_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.SHOB_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SHOB_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=shob-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.SHOB_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.SHOB_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.SHOB_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=shob-test")
