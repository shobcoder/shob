...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips shob-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-shob-directory"]).toBeUndefined()
      expect(forwarded["x-shob-workspace"]).toBeUndefined()
