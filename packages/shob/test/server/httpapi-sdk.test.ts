...
import { createOpencodeClient } from "@shob/sdk/v2"
import { validateSession } from "@/server/validate-session"
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.SHOB_SERVER_PASSWORD = input?.password
      Flag.SHOB_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".shob", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-shob-directory")).toBe(false)
        expect(request!.headers.has("x-shob-workspace")).toBe(false)
      }),
    ),
  )
