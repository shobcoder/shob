...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-shob-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-shob-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://shob.olova.dev/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-shob-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-shob-workspace")
