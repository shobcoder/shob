...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("SHOB_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.shob, ModelV2.ID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("nvidia")].options.headers).toEqual({
      "HTTP-Referer": "https://shob.olova.dev/",
      "X-Title": "opencode",
      "X-BILLING-INVOKE-ORIGIN": "Shob",
    })
  }),
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".shob")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".shob")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
...
it.effect("shob loader keeps paid models when config apiKey is present", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped({
      config: { provider: { shob: { options: { apiKey: "test-key" } } } },
    })
...
it.effect("shob loader keeps paid models when auth exists", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
    ...
    yield* Effect.acquireRelease(
      Effect.promise(() => Filesystem.write(authPath, JSON.stringify({ shob: { type: "api", key: "test-key" } }))),
      () =>
