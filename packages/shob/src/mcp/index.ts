...
import { ConfigMCPV1 } from "@shob/core/v1/config/mcp"
...
import { McpEvent } from "@shob/schema/mcp-event"
...
export class Service extends Context.Service<Service, Interface>()("@shob/MCP") {}
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: shob mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
