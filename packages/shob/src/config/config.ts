...
import { serviceUse } from "@shob/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@shob/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@shob/core/v1/config/console-state"
import { FSUtil } from "@shob/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@shob/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@shob/core/v1/config/config"
import { RemoteAuthError } from "@shob/core/v1/config/error"
import { ConfigPermissionV1 } from "@shob/core/v1/config/permission"
...
function globalConfigFile() {
  const candidates = ["shob.jsonc", "shob.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://shob.olova.dev/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SHOB_CONFIG && !Flag.SHOB_CONFIG_DIR && !Flag.SHOB_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://shob.olova.dev/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "shob.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "shob.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://shob.olova.dev/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/shob`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://shob.olova.dev/config.json"
        ...
        if (Flag.SHOB_CONFIG) {
          yield* merge(Flag.SHOB_CONFIG, yield* loadFile(Flag.SHOB_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SHOB_CONFIG })
        }
        ...
        if (Flag.SHOB_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SHOB_CONFIG_DIR", { path: Flag.SHOB_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".shob") || dir === Flag.SHOB_CONFIG_DIR) {
            for (const file of ["shob.json", "shob.jsonc"]) {
          ...
          // Auto-discovered plugins under `.shob/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (process.env.SHOB_CONFIG_CONTENT) {
          ...
          yield* Effect.logDebug("loaded custom config from SHOB_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              ...
              yield* env.set("SHOB_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.SHOB_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("SHOB_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
