...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@shob/sdk/v2"
...
export const AuthMethodID = "shob-login"
...
export class Service extends Context.Service<Service, Interface>()("@shob/ACP/Service") {}
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, shob provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const shobProvider = providers[ProviderV2.ID.make("opencode")]
  const shobModel = shobProvider ? Provider.sort(Object.values(shobProvider.models))[0] : undefined
  if (shobProvider && shobModel) return { providerID: shobProvider.id, modelID: shobModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Shob service failure", service })
}
