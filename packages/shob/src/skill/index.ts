...
import { FrontmatterError } from "@shob/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@shob/core/util/glob"
...
/** Global user skill store (same layout original shob desktop installs into). */
const SHOB_EXTERNAL_DIR = ".shob"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  // Installed built-ins live under ~/.shob/skills (desktop skill store).
  // Also discover ~/.claude/skills and ~/.agents/skills when enabled.
  const externalDirs: string[] = []
  if (!disableExternalSkills) {
    if (!disableClaudeCodeSkills) externalDirs.push(CLAUDE_EXTERNAL_DIR)
    externalDirs.push(AGENTS_EXTERNAL_DIR)

    const globalDirs = [SHOB_EXTERNAL_DIR, ...externalDirs]
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, SHOB_SKILL_PATTERN)
  }
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Re-scan disk so desktop install/uninstall into ~/.shob/skills is visible
