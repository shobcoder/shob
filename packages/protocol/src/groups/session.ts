...
import { PromptInput } from "@shob/schema/prompt-input"
import { Session } from "@shob/schema/session"
import { Project } from "@shob/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@shob/schema/schema"
...
import { SessionEvent } from "@shob/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Shob process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Shob process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
