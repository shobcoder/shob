...
import { useI18n } from "@shob/ui/context/i18n"
...
function formatCodeLanguage(language: string | undefined) {
  ...
  if (!value || value === "text" || value === "plaintext" || value === "plain") return ""
  return value
}

function ensureCodeHeader(wrapper: HTMLElement, language: string | undefined, labels: CopyLabels) {
  let header = wrapper.querySelector(':scope > [data-slot="markdown-code-header"]')
  if (!(header instanceof HTMLElement)) {
    ...
    if (first) wrapper.insertBefore(header, first)
    else wrapper.appendChild(header)
  }

  const label = header.querySelector('[data-slot="markdown-code-language"]')
  if (label instanceof HTMLElement) label.textContent = formatCodeLanguage(language)

  const buttons = Array.from(header.querySelectorAll('[data-slot="markdown-copy-button"]')).filter(
    (el): el is HTMLElement => el instanceof HTMLElement,
  )
  if (buttons.length === 0) header.appendChild(createCopyButton(labels))
  for (const button of buttons.slice(1)) {
    disposeCopyButton(button)
    button.remove()
  }

  // Legacy: remove copy buttons that lived as direct children of the code wrapper.
  for (const stray of Array.from(wrapper.children)) {
    if (!(stray instanceof HTMLElement)) continue
    if (stray.getAttribute("data-slot") !== "markdown-copy-button") continue
    disposeCopyButton(stray)
    stray.remove()
  }
}

function applyCodeMetadata(wrapper: HTMLElement, language: string | undefined, labels?: CopyLabels) {
  const isNewLayout = document.body.hasAttribute("data-new-layout")
  ...
  if (labels) ensureCodeHeader(wrapper, isNewLayout ? language : undefined, labels)
  else {
    const label = wrapper.querySelector('[data-slot="markdown-code-language"]')
    if (label instanceof HTMLElement) label.textContent = isNewLayout ? formatCodeLanguage(language) : ""
  }
}
...
function ensureCodeWrapper(block: HTMLPreElement, labels: CopyLabels) {
  ...
  if (!wrapped) {
    ...
    applyCodeMetadata(wrapper, codeLanguage(block), labels)
    return
  }

  applyCodeMetadata(parent, codeLanguage(block), labels)
}
...
function updateCodeBlock(
  ...
  if (code instanceof HTMLElement) {
    ...
    if (wrapper instanceof HTMLElement) applyCodeMetadata(wrapper, block.language, labels)
  ...
  applyCodeMetadata(wrapper, block.language, labels)
