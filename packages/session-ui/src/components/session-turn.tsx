...
import { activitySnapshotForVisibleParts } from "./session-activity"
...
export function SessionTurn(
  ...
  const assistantDerived = createMemo(() => {
    ...
    const activity = activitySnapshotForVisibleParts(allParts)
    return {
      visible,
      reason,
  ...
  const activityTitle = createMemo(() => assistantDerived().activityTitle)
  const activityVerb = createMemo(() => assistantDerived().activityVerb)
  const activityPath = createMemo(() => assistantDerived().activityPath)
  const activityKind = createMemo(() => assistantDerived().activityKind)
            ...
            >
              ...
              <Show when={showThinking()}>
                <div
                  ...
                  data-activity={activityTitle()}
                  data-activity-kind={activityKind()}
                  data-activity-path={activityPath() || undefined}
                  aria-live="polite"
                  aria-label={activityTitle()}
                >
                  <div data-slot="session-turn-thinking-main">
                    <TextShimmer text={activityVerb()} class="session-turn-thinking-label" />
                    <Show when={activityPath()}>
                      <span data-slot="session-turn-thinking-path" title={activityPath()}>
                        {activityPath()}
                      </span>
                    </Show>
                  </div>
