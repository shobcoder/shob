...
import { type UiI18n, useI18n } from "@shob/ui/context/i18n"
import { BasicTool, GenericTool } from "./basic-tool"
import { Accordion } from "@shob/ui/accordion"
import { StickyAccordionHeader } from "@shob/ui/sticky-accordion-header"
import { Collapsible } from "@shob/ui/collapsible"
import { FileIcon } from "@shob/ui/file-icon"
import { Icon } from "@shob/ui/icon"
import { ToolErrorCard } from "./tool-error-card"
import { Checkbox } from "@shob/ui/checkbox"
import { DiffChanges } from "@shob/ui/diff-changes"
import { Markdown } from "./markdown"
import { ImagePreview } from "@shob/ui/image-preview"
import { getDirectory as _getDirectory, getFilename } from "@shob/core/util/path"
import { checksum } from "@shob/core/util/encode"
...
import { TooltipV2 } from "@shob/ui/v2/tooltip-v2"
...
import type { IconProps } from "@shob/ui/icon"
...
export function renderable(part: PartType, showReasoningSummaries = true) {
  if (part.type === "tool") {
    return toolTranscriptVisible(part)
  }
              ...
              {(() => {
                const items = createMemo(
                  () => {
                    ...
                    return entry.refs
                      .map((ref) => ({
                        message: msgs().get(ref.messageID),
                        part: part().get(ref.messageID)?.get(ref.partID),
                      }))
                      .filter(
                        (item): item is { message: AssistantMessage; part: ToolPart } =>
                          !!item.message && item.part?.type === "tool",
                      )
                  },
                )
                ...
                return (
                  <Show when={items().length > 0}>
                    <ToolActivityGroup parts={items().map((item) => item.part)} busy={busy()}>
                      <Index each={items()}>
                        {(item) => (
                          <Part
                            part={item().part}
...
function toolActivityIcon(part: ToolPart | undefined): ToolInfo["icon"] {
  if (part?.tool === "task" && part.state.input?.subagent_type === "explore") return "magnifying-glass-menu"
  switch (part?.tool) {
    case "edit":
    case "write":
    case "apply_patch":
...
function activityItemTitle(props: ToolProps, fallback: string, i18n: UiI18n) {
  if (!props.activityItem) return fallback
  const pending = props.status === "pending" || props.status === "running"
  const kind =
    props.tool === "bash"
      ? "command"
      : props.tool === "edit" || props.tool === "write" || props.tool === "apply_patch"
        ? "edit"
        : props.tool === "read"
          ? "read"
          : props.tool === "list"
            ? "list"
            : props.tool === "glob" || props.tool === "grep"
              ? "search"
              : props.tool === "webfetch"
                ? "fetch"
                : props.tool === "websearch"
                  ? "web"
                  : undefined
  if (!kind) return fallback
  return i18n.t(`ui.toolActivity.item.${kind}.${pending ? "running" : "done"}`)
}
...
export function ToolActivityGroup(props: {
  ...
  const open = () => pending() || (props.open ?? localOpen())
  const summary = createMemo(() => toolActivitySummary(props.parts, i18n))
  ...
  return (
    <Collapsible
      ...
      data-scope="tool-activity"
      data-timeline-part-ids={props.parts.map((part) => part.id).join(",")}
    >
      <Collapsible.Trigger>
        <div data-component="tool-activity-group-trigger">
          <span data-slot="tool-activity-group-icon">
            <Icon name={toolActivityIcon(props.parts[0])} size="small" />
          </span>
          <span data-slot="tool-activity-group-summary" title={summary()}>
            <TextShimmer text={summary()} active={pending()} />
          </span>
          <span data-slot="tool-activity-group-arrow">
            <Collapsible.Arrow />
          </span>
        </div>
      </Collapsible.Trigger>
      <Collapsible.Content>
        <div data-component="tool-activity-group-list">{props.children}</div>
      </Collapsible.Content>
    </Collapsible>
  )
}
  ...
  return (
    <Show when={component()}>
      <Dynamic
        ...
        turnDurationMs={props.turnDurationMs}
        useV2Actions={props.useV2Actions}
        activityItem={props.activityItem}
      />
    </Show>
  )
}
  ...
  return (
    <Show when={!hideQuestion()}>
      <div
        ...
        data-activity-item={props.activityItem ? "true" : undefined}
    ...
    return (
      <>
        <BasicTool
          ...
          icon="glasses"
          trigger={{
            title: activityItemTitle(props, i18n.t("ui.tool.read"), i18n),
  ...
  render(props) {
    ...
    return (
      <BasicTool
        ...
        icon="bullet-list"
        trigger={{
          title: activityItemTitle(props, i18n.t("ui.tool.list"), i18n),
  ...
  render(props) {
    ...
    return (
      <BasicTool
        ...
        icon="magnifying-glass-menu"
        trigger={{
          title: activityItemTitle(props, i18n.t("ui.tool.glob"), i18n),
          subtitle: getDirectory(props.input.path || "/"),
  ...
  render(props) {
    ...
    return (
      <BasicTool
        ...
        icon="magnifying-glass-menu"
        trigger={{
          title: activityItemTitle(props, i18n.t("ui.tool.grep"), i18n),
          subtitle: getDirectory(props.input.path || "/"),
          args,
        }}
      >
          ...
          <div data-slot="basic-tool-tool-info-structured">
            <div data-slot="basic-tool-tool-info-main">
              <span data-slot="basic-tool-tool-title">
                <TextShimmer
                  text={activityItemTitle(props, i18n.t("ui.tool.webfetch"), i18n)}
                  active={pending()}
                />
              </span>
  ...
  render(props) {
    ...
    return (
      <BasicTool
        ...
        icon="window-cursor"
        trigger={{
          title: activityItemTitle(props, title(), i18n),
          subtitle: query(),
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    return (
      <BasicTool
        icon={props.activityItem && props.input.subagent_type === "explore" ? "magnifying-glass-menu" : "task"}
        status={props.status}
        activityItem={props.activityItem}
        trigger={trigger()}
        hideDetails
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    return (
      <BasicTool
        {...props}
          icon={props.activityItem ? "terminal" : "console"}
        trigger={(open) => (
          <div data-slot="basic-tool-tool-info-structured">
            <div data-slot="basic-tool-tool-info-main">
              <span data-slot="basic-tool-tool-title">
                <TextShimmer text={activityItemTitle(props, i18n.t("ui.tool.shell"), i18n)} active={pending()} />
              </span>
  ...
  render(props) {
    ...
    return (
      <div data-component="edit-tool">
        <BasicTool
          {...props}
          icon={props.activityItem ? "pencil-line" : "code-lines"}
              ...
              <div data-slot="message-part-title-area">
                <div data-slot="message-part-title">
                  <span data-slot="message-part-title-text">
                    <TextShimmer
                      text={activityItemTitle(props, i18n.t("ui.messagePart.title.edit"), i18n)}
                      active={pending()}
                    />
                  </span>
                  <Show when={!pending()}>
            ...
            <div data-component="write-trigger">
              <div data-slot="message-part-title-area">
                <div data-slot="message-part-title">
                  <span data-slot="message-part-title-text">
                    <TextShimmer
                      text={activityItemTitle(props, i18n.t("ui.messagePart.title.write"), i18n)}
                      active={pending()}
                    />
                  </span>
                  <Show when={!pending()}>
        ...
        fallback={
          <div data-component="apply-patch-tool">
            <BasicTool
              ...
              defer={props.deferContent !== false}
              trigger={{
                title: activityItemTitle(props, i18n.t("ui.tool.patch"), i18n),
                subtitle: subtitle(),
              }}
            >
              ...
              <div data-component="edit-trigger">
                <div data-slot="message-part-title-area">
                  <div data-slot="message-part-title">
                    <span data-slot="message-part-title-text">
                      <TextShimmer
                        text={activityItemTitle(props, i18n.t("ui.tool.patch"), i18n)}
                        active={pending()}
                      />
                    </span>
                    <Show when={!pending()}>
