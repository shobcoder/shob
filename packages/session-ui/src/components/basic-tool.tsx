...
import { Icon, type IconProps } from "@shob/ui/icon"
...
export function GenericTool(props: {
  ...
  const i18n = useI18n()
  const icon = () => {
    if (!props.activityItem) return "mcp" as const
    if (props.tool === "explore" || props.tool === "grep" || props.tool === "glob")
      return "magnifying-glass-menu" as const
    if (props.tool === "read") return "glasses" as const
    if (props.tool === "write" || props.tool === "edit" || props.tool === "apply_patch") return "pencil-line" as const
    if (props.tool === "shell" || props.tool === "bash") return "terminal" as const
    return "mcp" as const
  }
