import type { Todo } from "@shob-ai/sdk/v2"

export const composerEvent = "shob:e2e:composer"
...
export const composerEnabled = () => {
  ...
  return (window as ComposerWindow).__shob_e2e?.composer?.enabled === true
}

const root = () => {
  if (!composerEnabled()) return
  const state = (window as ComposerWindow).__shob_e2e?.composer
  if (!state) return
  state.sessions ??= {}
